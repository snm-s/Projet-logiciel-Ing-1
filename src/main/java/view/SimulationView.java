    ...
    */
    ...
    private VBox buildPanelLegend() {
        ...
        leg.getChildren().addAll(
            ...
            colorRow(RED,    "Zone inondée"),
            colorRow(YELLOW, "Sélectionné"),
    
            new Separator(),
    
            lbl("Routes", FontWeight.BOLD, 9, MUTED),
            colorRow(TEAL,   "Route sûre"),
            colorRow(ORANGE, "Route à risque"),
            colorRow(RED,    "Route inondée"),
    
            new Separator(),
    
            lbl("Agents", FontWeight.BOLD, 9, MUTED),
            colorRow(BLUE,   "Citoyen / PMR"),
            colorRow(GREEN,  "En sécurité / Secouriste"),
            colorRow(RED,    "Bloqué / stressé")
        );
    ...
    private HBox buildBottomBar() {
        ...
        lblSures = val("0%", TEAL);
lblRisque = val("0%", ORANGE);
lblCong  = val("0%", YELLOW);
lblOver  = val("0%", RED);
lblInond = val("0%", RED);
        b2.getChildren().addAll(
            barRow("Sûres",      TEAL,   lblSures),
            barRow("À risque",   ORANGE, lblRisque),
            barRow("Congestion", YELLOW, lblCong),
            barRow("Bloquées",   RED,    lblOver),
            barRow("Inondées",   RED,    lblInond));
    ...
    private void refreshUI() {
        ...
        try {
            ...
            if (ctrl != null) {
                ...
                long totalAgents = modele.getAgents().size();

                boolean evacuationLancee =
                simRunning
                || ctrl.getNombreAgentsEnDeplacement() > 0
                || modele.getTempsEcoule() > 0;
        
        long citoyensAEvacuer = evacuationLancee
            ? modele.getAgents().stream()
...
long citoyensEnSecurite = modele.getAgents().stream()
    .filter(a -> a instanceof Citizen)
    .map(a -> (Citizen) a)
    .filter(c -> c.getState() == CitizenState.SAFE)
    .count();

lblAgentsActifs.setText(String.valueOf(totalAgents));
lblPopRisque.setText(String.valueOf(citoyensAEvacuer));
lblPersonnesSec.setText(String.valueOf(citoyensEnSecurite));
            }
    ...
    private void syncMapAgents() {
        ...
        if (mapCtrl != null) {
            mapCtrl.syncAgents(modele.getAgents());
        }
